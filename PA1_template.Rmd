...
The median day recorded `r MedianStepsPerDay` steps and the mean number of steps per day was `r round(MeanStepsPerDay,0)`. These are shown by the vertical lines on the above plot.
...
The 5 minute interval with the highest average step count per day is `r MaxStepsInterval` which is more conventionaly displayed as `r round(MaxStepsInterval/100,0)`:`r MaxStepsInterval%%100` (h:mm). lol, perhaps that corresponds to the daily dog walk.
...
There are `r NumStepNA` NA values in steps and also `r NumNA` NA values in the entire dataset so that is the number of rows with NA values.
...
With the imputed values replacing the missing values, the median day recorded `r as.integer(round(NonMedianStepsPerDay,0))` steps and the mean number of steps per day was `r as.integer(round(NonMeanStepsPerDay,0))`. These are shown by the vertical lines on the above plot. The median didn't change with the imputation but now the mean is higher than the median, rather than lower!
